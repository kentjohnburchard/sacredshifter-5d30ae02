...
import { Heart, Sparkles, Music2, Lotus, Moon, Star } from "lucide-react";
...
const FrequencyInfoBox: React.FC<FrequencyInfoBoxProps> = ({
  ...
  const getBenefitIcon = (benefit: string, index: number) => {
    ...
    if (benefit.toLowerCase().includes("heal") || benefit.toLowerCase().includes("heart") || benefit.toLowerCase().includes("love")) {
      ...
      return <Lotus className={iconClasses} />;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1">
          <div className="mb-3">
            <label htmlFor="frequency-select" className="block text-sm font-medium text-slate-200 mb-2">
          ...
          <div className={`p-4 rounded-lg bg-gradient-to-r ${selectedFrequency.color} bg-opacity-20 shadow-lg shadow-black/40 border border-white/10`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-xl font-medium text-white drop-shadow-sm">{selectedFrequency.name}</h4>
            </div>
            <div className="flex items-center gap-1 mb-3">
              <span className="text-lg font-medium text-white/90">{selectedFrequency.frequency} Hz</span>
        ...
        <div className="md:col-span-2">
          <Tabs defaultValue="about" className="bg-black/50 backdrop-blur-md border border-white/10 rounded-lg shadow-lg">
            <TabsList className="grid grid-cols-3 bg-transparent border-b border-white/10 w-full rounded-t-lg">
              <TabsTrigger value="about" className="data-[state=active]:bg-white/20 data-[state=active]:text-white text-slate-200">
                <Music2 className="h-4 w-4 mr-2" />
                <span>About</span>
              </TabsTrigger>
              <TabsTrigger value="benefits" className="data-[state=active]:bg-white/20 data-[state=active]:text-white text-slate-200">
                ...
                {selectedFrequency.meditations ? (
                  <ul className="space-y-2">
                    {selectedFrequency.meditations.map((meditation, index) => (
                      <li key={index} className="flex items-start">
                        <Lotus className="h-5 w-5 mr-2 text-purple-300 shrink-0 mt-0.5" />
