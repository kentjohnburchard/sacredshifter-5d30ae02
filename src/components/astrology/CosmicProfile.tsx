  ...
  useEffect(() => {
    const getHoroscope = async () => {
      try {
        ...
        toast.error("Unable to fetch your cosmic insights. The stars are aligning, please try again soon.");
      } finally {
        setLoading(false);
      }
    };
        ...
        <CardContent className="p-6">
          {loading ? (
            ...
            <motion.div 
              className="space-y-6"
              initial="hidden"
              animate={fetchComplete ? "show" : "hidden"}
              variants={container}
            >
              {/* Date Range */}
              ...
              {/* Sound Recommendation - Optional Enhancement */}
              <motion.div variants={item}>
