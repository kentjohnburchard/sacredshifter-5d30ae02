...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  // Define theme-based border color - MUCH more visible and properly themed
  const borderColorClass = liftTheVeil 
    ? 'border-pink-500' // Solid pink border when veil is lifted - removed opacity
    : 'border-purple-500'; // Solid purple border otherwise - removed opacity

  const renderContent = () => (
    <div className={`flex flex-col min-h-screen border-[8px] ${borderColorClass} transition-colors duration-300`}>
  ...
  if (useBlueWaveBackground) {
    return (
      <div className={`flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 border-[8px] ${borderColorClass} transition-colors duration-300`}>
