...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const borderColorClass = liftTheVeil 
    ? 'border-pink-500/80' // More visible pink border when veil is lifted
    : 'border-purple-500/80'; // More visible purple border otherwise

  const renderContent = () => (
    <div className={`flex flex-col min-h-screen border-[6px] ${borderColorClass} transition-colors duration-300`}>
  ...
  if (useBlueWaveBackground) {
    return (
      <div className={`flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 border-[6px] ${borderColorClass} transition-colors duration-300`}>
