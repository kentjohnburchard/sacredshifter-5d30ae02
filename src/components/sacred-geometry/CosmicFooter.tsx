...
const CosmicFooter: React.FC<CosmicFooterProps> = ({
  ...
  return (
    <motion.footer
      ...
      className="fixed bottom-0 w-full bg-black/70 backdrop-blur-lg border-t border-purple-900/50 text-white py-2 px-4 z-50"
    >
      {showFrequencyBar && currentFrequency && (
        <motion.div
          ...
          className="relative h-1 w-full mb-3"
        >
      ...
      <div className="flex items-center justify-center space-x-2 md:space-x-6">
        {footerLinks.map((link) => (
          <Link 
            key={link.name}
            to={link.path}
            className="group relative flex flex-col items-center justify-center text-xs"
          >
            <motion.div 
              whileHover={{ y: -2 }}
              className="flex items-center justify-center p-1.5 rounded-full bg-gradient-to-br from-purple-900 to-indigo-900 group-hover:from-purple-700 group-hover:to-indigo-700"
            >
              {link.icon}
              <motion.span 
                className="absolute -top-8 bg-black/80 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap"
                animate={{ y: [0, -2, 0] }}
            ...
            <span className="mt-1 opacity-80 group-hover:opacity-100">{link.name}</span>
          </Link>
        ))}
      </div>
    </motion.footer>
