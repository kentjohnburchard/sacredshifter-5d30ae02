...
// Optimized display component with minimal animations
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* Shadow Frequencies - Simplified */}
              <div className={`p-4 rounded-lg ${liftTheVeil ? 'bg-purple-900/50' : 'bg-indigo-100/30 backdrop-blur-md'}`}>
                <h3 className={`font-medium ${liftTheVeil ? 'text-white' : 'text-white'}`}>
                  Shadow Frequencies
                </h3>
                <div className="mt-2">
                  <p className={`${liftTheVeil ? 'text-gray-300' : 'text-gray-200'}`}>
                    {blueprint.shadow_frequencies.slice(0, 2).join(', ')}
                  </p>
                </div>
              </div>
              
              {/* Simplified Sacred Blueprint Text */}
              <div className={`p-4 rounded-lg ${liftTheVeil ? 'bg-purple-900/40' : 'bg-indigo-100/30 backdrop-blur-md'}`}>
                <h3 className={`font-medium mb-2 ${liftTheVeil ? 'text-white' : 'text-white'}`}>
                  Your Sacred Story
                </h3>
                <p className={`text-sm ${liftTheVeil ? 'text-gray-300' : 'text-gray-200'}`}>
                  {blueprint.blueprint_text.substring(0, 300)}...
                </p>
              </div>
            </div>
          </div>
...
export default memo(BlueprintDisplay);
